<?php
/* SVN FILE: $Id: soft_deletable.php 49 2008-06-16 16:39:52Z mgiglesias $ */
...
class SoftDeletableBehavior extends ModelBehavior {
	...
	function beforeDelete(&$Model, $cascade = true) {
		if ($this->__settings[$Model->alias]['delete'] && $Model->hasField($this->__settings[$Model->alias]['field'])) {
			...
			$data = array($Model->alias => array(
				$attributes['field'] => date('Y-m-d H:i:s')
			));
	...
	function purge(&$Model, $cascade = true) {
		...
		if ($Model->hasField($this->__settings[$Model->alias]['field'])) {
			...
			$purged = $Model->deleteAll(array('NOT' => array($this->__settings[$Model->alias]['field'] => null)), $cascade);
	...
	function undelete(&$Model, $id = null, $attributes = array()) {
		if ($Model->hasField($this->__settings[$Model->alias]['field'])) {
			...
			$data = array($Model->alias => array(
				$Model->primaryKey => $id,
				$this->__settings[$Model->alias]['field'] => null
			));
		...
		if ($this->__settings[$Model->alias]['find'] && $Model->hasField($this->__settings[$Model->alias]['field'])) {
			...
			if ($include) {
				...
				if (is_string($queryData['conditions'])) {
					$queryData['conditions'] = $Db->name($Model->alias) . '.' . $Db->name($this->__settings[$Model->alias]['field']) . 'IS NULL AND ' . $queryData['conditions'];
				} else {
					$queryData['conditions'][$Model->alias . '.' . $this->__settings[$Model->alias]['field']] = null;
				}
			}
		}
