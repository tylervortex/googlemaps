...
class GeocodableBehavior extends ModelBehavior {
	...
	protected $services = array(
		'google' => array(
			...
			'format' => '${address} ${address1} ${address2} ${city} ${zip} ${state} ${country}',
			'pattern' => '/200,[^,]+,([^,]+),([^,\s]+)/',
			'matches' => array(
				'latitude' => 1,
				'longitude' => 2
	...
	public function beforeSave($model) {
		...
		if (!empty($latitudeField) && !empty($longitudeField)
		   	&& (!isset($model->data[$model->alias][$latitudeField]) || empty($model->data[$model->alias][$latitudeField]))
		   	&& (!isset($model->data[$model->alias][$longitudeField]) || empty($model->data[$model->alias][$longitudeField]))) {
	...
	public function geocode($model, $address, $save = false) {
		...
		if (!empty($settings['fields']['full_address']) && !empty($address['full_address'])) {
			$fullAddress = $address['full_address'];
		} else {
			$fullAddress = $this->_fullAddress($settings, $address);
		}
		...
		} else if (!empty($settings['fields']['full_address'])) {
			$conditions[$model->alias . '.' . $settings['fields']['full_address']] = $fullAddress;
		}

		if (!empty($settings['fields']['full_address'])) {
			$data[$model->alias][$settings['fields']['full_address']] = $fullAddress;
		}

		if (is_array($address)) {
			foreach (array_intersect_key($address, $settings['fields']) as $field => $value) {
				if (empty($settings['fields']['hash']) && empty($settings['fields']['full_address'])) {
	 ...
	 */
	protected function _fetchCoordinates($settings, $fullAddress) {
		$vars = array(
			'${key}' => $settings['key'],
			'${address}' => $fullAddress
		);
	 ...
	 */
	protected function _fullAddress($settings, $address) {
		$fullAddress = $address;
		if (is_array($address)) {
			$elements = array();
			...
			$fullAddress = trim(str_replace(array_keys($elements), $elements, $this->services[$settings['service']]['format']));
			$replacements = array(
				'/(\s)\s+/' => '\\1',
				'/\s+,(.+)/' => ',\\1',
				'/\s*,\s*,/' => ',',
			...
			foreach ($replacements as $pattern => $replacement) {
				$fullAddress = preg_replace($pattern, $replacement, $fullAddress);
			}
