...
class GeomapHelper extends AppHelper {
	...
	protected function _google($id, $center, $markers, $parameters) {
		...
		if ($parameters['version'] >= 3) {
			return $this->_google_v3($id, $center, $markers, $parameters);
		} else if ($parameters['version'] == 2) {
			return $this->_google_v2($id, $center, $markers, $parameters);
		}
	}

	protected function _google_v2($id, $center, $markers, $parameters) {
		$mapTypes = array(
		...
		$varName = 'm' . $id;
		$script = 'var ' . $varName . 'Callback = function() { ';

		$script .= 'if (!google.maps.BrowserIsCompatible()) { return false; } ';
		$script .= 'var mapOptions = {}; ';

		if (!empty($parameters['width']) && !empty($parameters['height'])) {
			$script .= 'mapOptions.size = new google.maps.Size(' . $parameters['width'] . ', ' . $parameters['height'] . '); ';
		}
		...
		if (!empty($center)) {
			...
			$script .= 'var center = new google.maps.LatLng(' . $latitude . ', ' . $longitude . '); ';
			if (!empty($parameters['zoom'])) {
				$script .= $varName . '.setCenter(center, ' . $parameters['zoom'] . '); ';
			} else {
				$script .= $varName . '.setCenter(center); ';
			}
		}
		...
		if (!empty($markers)) {
			foreach ($markers as $marker) {
				...
				$script .= 'var point = new GLatLng(' . $latitude . ', ' . $longitude . '); ';
				$script .= 'var markerOptions = {}; ';

				if (!empty($markerOptions['icon'])) {
					$script .= 'markerIcon = new google.maps.Icon(G_DEFAULT_ICON); ';
					$script .= 'markerIcon.image = "' . $markerOptions['icon'] . '"; ';
					$script .= 'markerOptions.icon = markerIcon; ';
				}

				$script .= 'var marker = new google.maps.Marker(point, markerOptions); ';
				if (!empty($content)) {
					$script .= 'google.maps.Event.addListener(marker, "click", function() { ';
					$script .= 'marker.openInfoWindowHtml("' . $content . '"';
					if (!empty($markerOptions['maxWidth'])) {
						$script .= ', {maxWidth: ' . $markerOptions['maxWidth'] . '} ';
					}
					$script .= ')}); ';
				}

				$script .= $varName . '.addOverlay(marker); ';
				$script .= $varName . '.__version__ = ' . $parameters['version'] . '; ';

			}
		}

		$script .= '}; ';
		$script .= 'var ' . $varName . ' = null; ';
		$script .= 'google.load("maps", "' . $parameters['version'] . '", { ';
		$script .= 'other_params: "sensor=' . (!empty($parameters['sensor']) ? 'true' : 'false') . '", ';

		if (!empty($parameters['language'])) {
			$script .= 'language: "' . $parameters['language'] . '", ';
		}

		$script .= 'callback: "' . $varName . 'Callback" ';
		$script .= '}); ';

		return $this->Javascript->codeBlock($script);
	}

	protected function _google_v3($id, $center, $markers, $parameters) {
		$mapTypes = array(
		...
		if (!empty($markers)) {
			foreach ($markers as $marker) {
				...
				$script .= 'marker = new GMarker(new GLatLng(' . $latitude . ', ' . $longitude . '), markerOptions);';
				$script .= $varName . '.addOverlay(marker);';
